...
- Visualize the distribution of `synergy`. 
  ...
  - Then calculate the following summary statistics from the data, the mean, standard deviation, an sample size. Save the data frame of summary statistics as `summary_stat`. Be sure to still display the values of the summary statistics.
...
3. *Is the synergy in burritos generally pretty good?* To assess this, we will conduct a hypothesis test to evaluate if the mean synergy is greater than 3. Before conducting inference, we need to check the conditions to make sure the Central Limit Theorem can be applied in this analysis. For each condition, indicate whether it is satisfied and provide a brief explanation supporting your response. 

     - Independence? 
     - Sample size / distribution? 
    
4. State the null and alternative hypotheses to evaluate the question posed in the previous exercise in words and in statistical notation. 
...
Use the `summary_stat` data frame to calculate and display these values. 
6. Next, use R as a calculator to calculate the test statistic, $T$. Recall 
...
7. Lastly let's look at the p-value. 
...
8. Now let's calculate a 90% confidence interval for the mean synergy score of all burritos. The confidence interval for a population mean is 
...
For example, if we want to calculate a 95% confidence interval for the mean, we will use `qt(0.975, n-1)`, where 0.975 is the cumulative probability at the upper bound of the 95% confidence interval (recall we used this value to find the upper bound when using bootstrap intervals), and n-1 are the degrees of freedom. 

- Calculate the critical value, $t^*_{n-1}$, for the 90% confidence interval for the mean synergy score of all burritos. 

- Use R as a "calculator" to calculate the 90% confidence interval. 

- Interpret the interval in the context of the data. 

9. We can conduct CLT-based inference for a population mean using the `t_test` function in the `infer` R package. 

- Fill in the code below to produce the test statistic and p-value to test the hypotheses in Exercise 4. 
...
```
  ...
  The results should be the same as the calculations you did in exercises 6 and 7.
    
- Fill in the code below to produce the 90% confidence interval for the mean synergy score. The results should be the same as the calculations from Exercise 8.
...
- What is one way conducting hypothesis tests is the same in both approaches? What is one way conducting hypothesis tests differ between the two approaches?
